...
public class Heap<V extends Comparable<? super V>>{
     ...
     * @throws Exception
     */
    public void insert(V item) throws Exception {
        int position;
        heapSize++;
        ...
        // greater than the element or the item reaches the root(index 0).
        while (position > 0
                && data.get(position).compareTo(data.get(parentPosition(position))) > 0) {
    ...
    public void fixHeap(int heapSize) throws Exception {
        ...
        while (leftChildPosition(position) < heapSize) {
            ...
            if (childPosition < heapSize - 1 //check if right child exists
                    && data.get(childPosition + 1).compareTo(data.get(childPosition)) > 0) // right child is greater than left child
            {
                childPosition++;
            }
            if (data.get(position).compareTo(data.get(childPosition)) < 0) {
